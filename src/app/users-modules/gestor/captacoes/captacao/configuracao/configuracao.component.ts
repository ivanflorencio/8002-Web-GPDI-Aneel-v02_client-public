...
export class ConfiguracaoComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.captacao.termino != null) {
            ...
            this.dataMaximaExt = this.captacao.termino;
    ...
    async anexarArquivos() {
        try {
            this.uploads = await this.app.uploadForm(
                this.uploads.map((f) => f.id),
                `Captacoes/${this.captacao.id}/Arquivos`
            );
            const files = this.uploads.map((f) => f.id);
            this.arquivosControls.setValue(files);
        } catch (e) {
    ...
    async cancelarCaptacao() {
        const btnOptions = [
            { text: 'Cancelar', value: false, cssClass: 'btn btn-link' },
            {
                ...
                value: true,
                cssClass: 'btn-danger',
                checkMessage: 'Confirmo que estaremos encerrando esta captação',
            },
        ];

        if (
            await this.app.confirm(
                'Esta ação é irreversível. Todos os fornecedores convidados irão receber um email ' +
                    'avisando deste encerramento. Confirmar encerramento?',
                'TEM CERTEZA QUE DESEJA ENCERRAR ESTA CAPTAÇÃO?',
                btnOptions
            )
        ) {
            this.isLoading = true;
