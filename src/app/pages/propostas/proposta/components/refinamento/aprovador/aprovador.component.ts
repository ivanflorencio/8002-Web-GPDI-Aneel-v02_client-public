...
export class AprovadorComponent implements OnInit {
    ...
    async cancelarProcesso() {
        if (
            await this.app.confirm(
                'Esta ação é irreversível. Ao cancelar o processo de refinamento o fornecedor será notificado por e-mail',
    ...
    async solicitarNovaAnalise() {
        if (
            await this.app.confirm('Você está prester a enviar a proposta para re-análise.', 'ENCAMINHAR PARA RE-ANÁLISE?', [
                { text: 'Cancelar', value: false, cssClass: 'btn btn-link' },
                {
                    text: 'Solicitar Re-análise',
                    value: true,
                    cssClass: 'btn-primary',
                },
            ])
        ) {
            await this.service.post(`${this.proposta.guid}/SolicitarReanalise`, {});
            await this.app.alert('Proposta enviada para Re-análise');
            await this.router.navigate(['/refinamento']);
        }
    }
