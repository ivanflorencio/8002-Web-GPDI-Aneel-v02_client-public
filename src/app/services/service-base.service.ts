...
export class ServiceBase<T extends { id?: any }> extends UploadFilesService {
    ...
    put: (url, data, ...args) => Promise<any> = (url, data, ...args) =>
        this.http.put(`${this.controller}/${url}`, data, ...args).toPromise();
    post: (url, data, ...args) => Promise<any> = (url, data, ...args) =>
        this.http.post(`${this.controller}/${url}`, data, ...args).toPromise();
