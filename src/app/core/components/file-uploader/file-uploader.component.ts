...
export class FileUploaderComponent implements OnInit {
  ...
  async sendFiles() {
    try {
      ...
      this._preSelecteds = this.filesSent
        .filter((f) => this.selecteds[`${f.id}`])
        .map((f) => f.id);
      this.http
        .post(this.pathUpload, formdata, {
          reportProgress: true,
          observe: "events",
        })
        .subscribe(
          ...
          (error) => {
            console.error(error);
            this.app.alertError(
              "Erro no upload, verifique os arquivos permitidos e tente novamente"
            );
            this.progress = 0;
            this.isSending = false;
          }
  ...
  async deleteFile(event: MouseEvent, file) {
    ...
    if (
      await this.app.confirm(
        "Tem certeza que deseja excluir esse arquivo? Essa ação não pode ser desfeita",
        "Tem certeza?"
      )
    ) {
