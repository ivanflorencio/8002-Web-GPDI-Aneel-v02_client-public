...
export class TableComponent implements OnInit {
    ...
    @Input() set cols(value: TableComponentCols) {
        this.$cols = value;
        this.$defaultListOrder = this.cols
            .filter((c) => c.order)
            .map((c) => ({
                field: c.field,
                direction: 'asc',
    ...
    orderFromCols() {
        this.listOrder = uniqBy(
            [
                ...
                ...this.$cols.filter((c) => c.order).map((c) => ({ field: c.field, direction: 'asc' } as TableOrder)),
            ],
            (i) => i.field
        ).reverse();
    }
    ...
    orderRows($data: Array<TableComponentRow>) {
        this.listOrder.forEach((listOrder) => {
            ...
            switch (listOrder.type) {
                ...
                default:
                    interator = (item) =>
                        get(item, `value.${camelCase(listOrder.field)}`)
                            ?.toString()
                            .toLowerCase();
                    break;
            }
    ...
    async setCurrentData(page?: number) {
        ...
        if (this.dataRequester) {
            try {
                ...
                const response = await this.dataRequester(
    ...
    protected toImage(cellData: TableCellData, asBackground = false) {
        ...
        if (asBackground) {
            return this.sanitizer.bypassSecurityTrustHtml(
                `<div style="background-image:url(${value})" class="table-field-imagebg-${slug}" />`
            );
        }
    ...
    renderCell(cellData: TableCellData): string | SafeHtml {
        try {
            ...
            switch (cellData.col.type) {
                ...
                case 'template':
                    const text =
                        typeof cellData.col.template === 'function' ? cellData.col.template(cellData.value) : cellData.col.template;
